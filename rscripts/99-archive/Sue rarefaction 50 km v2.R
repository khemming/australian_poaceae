...
# raster
  raster <- raster("Australia/aus 50-km.grd")
  plot(raster)
...
# remove oceanic cells using template I made earlier (see 'Australia raster' script)
  cell.cat <- read.csv("Australia/aus 50-km.csv", header = T) %>%
...
# save data frames
  write.csv(x, "C:/Users/s436862/Dropbox/Rarefaction/Results/Rarefaction/Sue/CSV/50 km Rarefied native richnesss 10 to 50 records.csv", row.names = F)
