...
# load  workspace from iNEXT model (script) -------------------------------------
  load("data/supplementary materials/species gls model data.RData")
...
# r2 
  r2 <- read.csv("results/supplementary materials/species/r2.csv")
  r2
  ...
  coef_plot <- function(dat, subtribe, x_mn, x_mx, brks, r2_i){
    ...
    q <- ggplot(dat, aes(y = plot_names)) +
      ...
      theme(axis.title = element_text(size = 14),
        ...
        save <- paste0("results/coefficient plots/", subtribe, " (species).jpeg")
...
# breaks
  breaks <- list(c(-0.6, -0.3, 0, 0.3),         # Ancient
                 c(-0.6, -0.3, 0, 0.3),         # C3 recent
                 c(-0.6, -0.3, 0, 0.3),       # Early endemic
               
                 c(-0.4, -0.2, 0, 0.2, 0.4),     # Early native
                 c(-0.4, 0, 0.4, 0.8, 1.2),          # Home grown
                 c(-0.4, -0.2, 0, 0.2),       # Mid endemic
                 
                 c(-0.4, -0.2, 0, 0.2, 0.2))  # Mid native 
...
# grouped plots --------------------------------------------------
  coef_v1 <- function(title, a, b, x_limits, x_labels){
    ...
    q
    save <- paste0("Results/coefficient plots/", title, " (species).jpeg")
...
# Andropogoneae native/endemic
  coef_v1("Mid native-endemic", 
          ...
          "Mid_native",
          c(-0.5, 0.26), 
          c(round(seq(-0.4, 0.4, 0.2), digits = 2)))
