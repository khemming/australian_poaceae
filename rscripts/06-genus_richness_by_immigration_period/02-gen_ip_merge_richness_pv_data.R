...
# rasters
  ...
  names <- gsub(pattern = "results/supplementary materials/genera/|.grd$", "", current.list)
  ...
  write.csv(st, "results/supplementary materials/genera/raw generic richness 2538.csv", row.names = F)
...
# bind species and EV data and subset to terrestrial only cells   
  ...
  write.csv(st_pv, "results/supplementary materials/genera/genera pv 2538.csv", row.names = F)      
...
# subset land only cells (some EVs don't have values where there is very low proportion land cover)
  st_pv_red <- bind_cols(stl, pv) %>%
    filter(!is.na(clay))
  write.csv(st_pv_red, "results/supplementary materials/genera/genera pv 1146.csv", row.names = F)      
# --------------------------------------------------------------------------------
